...
# 🔹 Super Admin (Only this ID can add/remove users)
...
bot = Client("bot_session", api_id=12345, api_hash="your_api_hash", bot_token="your_bot_token")

# 🔹 Function to Process .sh File
def process_sh_file(input_file):
    output_file = input_file.replace(".sh", ".txt")

    title_pattern = r'(?<=-f ")([^"]+)'  # Matches titles (.mp4/.pdf)
...
# 🔹 Handle .sh File Uploads
@bot.on_message(filters.document & filters.private)
async def sh_file_handler(client, message):
    user_id = message.from_user.id
    if user_id not in AUTH_USERS:
        await message.reply_text("❌ You're not allowed to upload files!")
        return
    ...
    output_path = process_sh_file(file_path)
    
    if os.path.exists(output_path):
        await message.reply_document(output_path, caption="✅ Here is the extracted data.")
